...
static int mycrypto_init(void)
{
	...
	if (err == 0) {
		pr_debug("%s: CBC(AES-128) encryption[%d]:\n", __DRIVER_NAME, enc_len);
		print_hex_dump_debug("", DUMP_PREFIX_NONE, 16, 1, enc_data, enc_len, true);
		char b64_str[256]={0};
		//bintob64(b64_str,enc_data,enc_len);
		b64_encode(enc_data,enc_len,b64_str,b64_encode_size(enc_len));
		pr_debug("b64: %s\n", b64_str);
	}

	err = my_aes_cbc_decrypt_demo(key, key_sz, enc_data, enc_len, dec_data);
	if (err == 0) {
		pr_debug("%s: CBC(AES-128) decryption:[%s]\n", __DRIVER_NAME, dec_data);
